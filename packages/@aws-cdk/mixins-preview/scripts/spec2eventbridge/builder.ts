...
export class EventBridgeBuilder extends LibraryBuilder<EventBridgeServiceModule> {
  public static canBindToResource(db: SpecDatabase, event: Event, resource: Resource): boolean {
    const rootProperty = db.get('eventTypeDefinition', event.resourcesField[0].type);
    ...
    for (const [propName] of Object.entries(rootProperty.properties)) {
      ...
      if (!rscField.fieldName) {
        ...
        // Mirrors SkipEventError in getResolver: fieldName explicitly names this property but it's not in the ref
  ...
  protected createServiceSubmodule(service: Service, submoduleName: string): EventBridgeServiceModule {
    ...
    if (hasEvents.length != 0) {
      ...
      const hasBindableEvents = allServiceEvents.some(event => {
        ...
        return EventBridgeBuilder.canBindToResource(this.db, event, resource);
      });
  ...
  protected addResourceToSubmodule(submodule: EventBridgeServiceModule, resource: Resource, _props?: AddServiceProps): void {
    ...
    // Only include events that can actually bind to this resource; unbindable ones are handled as standalone
    const events = allEvents.filter(e => e.resourcesField.length === 0 || EventBridgeBuilder.canBindToResource(this.db, e, resource));
    if (events.length === 0) {
      return;
    }
  ...
  private generateStandaloneEvents(submodule: EventBridgeServiceModule, service: Service): void {
    ...
    const standaloneEvents = allServiceEvents.filter(event => {
      ...
      return !resources.some(r => EventBridgeBuilder.canBindToResource(this.db, event, r));
    });
...
class SharedConverterGenerator {
  ...
  createTypeConverterForNamespace(namespaceScope: ClassType,
    event: Event, eventNsName: string, hasResource: boolean, rootPropertyId?: string): TypeConverter {
  ...
  addPropertiesAndCreateConverter({
    ...
    // Only check for the root property - nested typeDefinitions have different $id and won't match resourcesField.
    const isRootProperty = this.context.db.get('eventTypeDefinition', event.rootProperty).$id === typeDef.$id;
    if (this.context.resource && isRootProperty) {
      const wouldSkip = event.resourcesField.some(r => r.type.$ref === typeDef.$id) && nonUndefinedResolvers !== 1;
      const canBind = EventBridgeBuilder.canBindToResource(this.context.db, event, this.context.resource);
      if (wouldSkip !== !canBind) {
        throw new Error(`canBindToResource mismatch for event ${event.name} resource ${this.context.resource.name}: wouldSkip=${wouldSkip}, canBind=${canBind}`);
      }
    }

    if (hasResource && event.resourcesField.some(r => r.type.$ref === typeDef.$id) && nonUndefinedResolvers !== 1) {
    ...
    for (const event of this.events) {
      try {
        ...
        if (error instanceof SkipEventError) {
          log.debug(`Skipping event ${event.name} for ${this.context.resource?.name ?? 'standalone'}: ${error.message}`);
          continue;
        }
        throw error;
      }
